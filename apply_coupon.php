...
if($check>0){
	...
	if($getcartTotalPrice>$cart_min_value){
		if($cur_time>$expired_on){
			$arr=array('status'=>'error','msg'=>'This Coupon has expired');
		}else{
			$coupon_code_apply=0;
			if($coupon_type=='F'){
			...
			$arr=array('status'=>'success','msg'=>'Coupon applied','field'=>'form_msg','coupon_code_apply'=>$coupon_code_apply);
		}
	}else{
		$arr=array('status'=>'error','msg'=>'Coupon only valid for cart value greater then Rs '.$cart_min_value);
	}
}else{
	$arr=array('status'=>'error','msg'=>'Invalid Coupon');
}
